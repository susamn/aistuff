...
# project bundle in the persistent data home, and re-stamps the multi-project
# report.html from everything in the store.
set -euo pipefail
...
Usage: run-report.sh [options]
  ...
  --render-only       Skip analysis; rebuild report.html from the existing data store
  ...
  --open              Open the report with xdg-open when done
EOF
  exit 1
}
...
# data_home: where per-project JSON bundles and the stamped report.html live
...
EOF
  echo "[config] Created $CONFIG_FILE with defaults (data_home, window_months, forge limits)."
fi
prop() { grep -E "^$1=" "$CONFIG_FILE" | tail -1 | cut -d= -f2- ; }
...
if [[ $RENDER_ONLY -eq 1 ]]; then
  $BUILD stamp "$SKILL_DIR/template/report.html" "$DATA_HOME/data" "$DATA_HOME/report.html"
  [[ $OPEN -eq 1 ]] && xdg-open "$DATA_HOME/report.html" >/dev/null 2>&1 || true
  exit 0
fi
...
$BUILD bundle "$RUN_TMP/meta.json" "$DATA_HOME/data/$SLUG.json" "${ENVELOPES[@]}"
$BUILD stamp "$SKILL_DIR/template/report.html" "$DATA_HOME/data" "$DATA_HOME/report.html"

echo
echo "[done] Project data : $DATA_HOME/data/$SLUG.json"
echo "[done] Report       : $DATA_HOME/report.html"
