...
8. **Audit** — `~/dotfiles/aistuff/skills/skill-manager/scripts/audit.sh <name>`.
   Must pass before committing.
9. **Commit** — `skills/` is a **git submodule**, so this is two commits:
   ```bash
   git -C ~/dotfiles/aistuff/skills add <name>/ AGENTS-TEMPLATE.md
   git -C ~/dotfiles/aistuff/skills commit -m "skills: add <name>"
